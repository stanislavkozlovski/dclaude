...
The design goal is path fidelity. The target repo and configured home folders are mounted into the container at the same absolute paths they have on the host. When no launcher config is present, the default home mounts remain `~/Desktop` and `~/Downloads`. `/workspace` exists only as a compatibility alias.

The runtime model uses two separate path roles:

- tool home: where this `dclaude` repo lives
- target repo: the git repo from the user's current shell location
...
- entries are `$HOME`-relative path suffixes such as `/Desktop`
...
- configured home folders at the same path, read-only
